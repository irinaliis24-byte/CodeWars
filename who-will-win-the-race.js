...
function whoWillWin(arr) {
    ...
    const racers = arr
        ...
        .filter(Boolean); //отбрасываем ложные значения (null)

    if (!racers.length) return "Everyone was disqualified!";

    const minTime = Math.min(...racers.map(r => r.time));
    const winners = racers.filter(r => r.time === minTime);
